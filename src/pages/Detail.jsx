  ...
  const quizBank = [
    {
      ...
      questions: [
        ...
        { q: '대한민국 임시정부가 세워진 곳은?', options: ['중국 상하이', '일본 도쿄', '미국 워싱턴', '러시아 모스크바'], answer: 0, explain: '1919년 중국 상하이에서 대한민국 임시정부가 세워졌어요.' },
      ],
    },
    {
...
const PROMPT_GALLERY = {
  ...
  middle: [
    { icon: '📊', label: '비교 분석', prompt: '"{title}"과 동시대 또는 유사 성격의 유산 3곳을 비교 분석해줘. 역사적 배경, 건축·전시 특징, 현대적 의의를 표로 정리하고 중학생 수준의 해설을 덧붙여줘.' },
    { icon: '🎭', label: '역사 토론', prompt: '"{title}"을 주제로 중학생이 참여할 수 있는 찬반 토론 주제 2개를 제안하고, 각 입장의 근거를 3가지씩 정리해줘.' },
    { icon: '📰', label: '다큐멘터리 기획', prompt: '"{title}"을 소재로 30분짜리 역사 다큐멘터리 기획안을 만들어줘. 구성(오프닝-본편-엔딩), 인터뷰 대상, 핵심 질문을 포함해줘.' },
...
export default function Detail() {
  ...
  return (
    <div className="detail">
      ...
      {detail.firstimage && <img src={detail.firstimage} alt={detail.title} className="detail-img" onError={(e) => { e.target.style.display='none'; }} />}
