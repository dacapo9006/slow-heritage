...
const AGE_CONFIG = {
  ...
  toddler: { maxStops: 3, maxWalk: 25, tip: '30분 이내 돀보 거리를 추천합니다. 쉴 곳이 있는지 확인하세요.' },
...
const CURRICULUM_MAP = {
  ...
  '성곽': { grade: '초등 5~6 / 중학교', code: '[6사05-02] [9역07-02]', desc: '조선 훈기 변화와 문화유산의 보존' },
  '고분': { grade: '초등 5~6 / 중학교', code: '[6사04-02] [9역02-01]', desc: '고대 국가의 형성과 삼국의 발전' },
  '서원·향교': { grade: '초등 5~6 / 중합교', code: '[6사05-01] [9역06-02]', desc: '유교 문화와 조선의 사회 질서' },
  '박물관': { grade: '초등 3~6 학년', code: '[4사01-01]', desc: '지역의 문화유산 탐군' },
  '전통럸을': { grade: '초등 3~4학년', code: '[4사02-01]', desc: '전통 생활 문화의 이해' },
  '근대역사': { grade: '중합교', code: '[9역14-01]', desc: '근대 국가 수립 노력과 민조 운동' },
};

/** 연령별 안전 체크리스트 */
const SAFETY_CHECKLISTS = {
  baby: [
    '유모차·기저귀 교환대 위치를 확인했나요?',
    '수유싷�뾌위·이유식 등필수 용품을 챉겼나요?',
    '수유싷·기저귀 교환대 위치를 확인했나요?',
    '자외선 차단제와 모자를 준비했나요?',
    '아이가 입에 넣을 수 있는 물건에 주의하세요.',
    '비상 연락처와 가까운 병원 위치를 확인했나요?',
  ],
  toddler: [
    '편한 신발을 신겼나요? (계닧·비탈기 맞음)',
    '물과 간식을 충분히 챉겼나요?',
  ...
  child: [
    '편한 운동화를 신었나요?',
    '물·간숝¹우산 등 기본 준비물을 확인했나요?',
    '문화재는 만지거나 오라타지 않기로 약속했나요?',
    '사진 추영 금지 구역을 함께 확인하세요.',
    '미션 수첩이나 메모장을 준비하면 좋아요!',
    '자외선 차단 + 모자를 잊지 마세요.',
  ],
  upper: [
    '충분한 물과 간식을 챉겼나요?',
    '문화재 보호 규칙을 함께 읽薴보았나요?',
  ...
  middle: [
    '편한 신발과 충분한 물·간숝을 준비했나요?',
    '맲㶔재 보호 규칙과 촬영 제한 구역을 확인했나요?',
            ...
            <div key={place.contentid} className="course-stop">
              {idx > 0 && segments[idx-1] && (
                <div className="course-travel">
                  ...
                  <span className="travel-info">
                    🚶 돀보 {segments[idx-1].walk}분 ({segments[idx-1].dist < 1 ? `${Math.round(segments[idx-1].dist*1000)}m` : `${segments[idx-1].dist.toFixed(1)}km`})
              ...
              <Link to={`/detail/${place.contentid}?typeId=${place.contenttypeid}&childAge=${childAge}`} className="stop-card">
                ...
                <div className="stop-info">
                  ...
                  <p className="stop-time">⏱ 추천 체륙 {childAge === 'baby' ? '20분' : '30~40분'}</p>
                </div>
              </Link>
            </div>
          ))}
